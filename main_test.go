...
func TestMain(t *testing.T) {
	...
	t.Run("ok", func(t *testing.T) {
		...
		t.Run("struct", func(t *testing.T) {
			cmd := exec.Command(testbin, "--type", "Color")
			cmd.Env = append(cmd.Environ(), "GOFILE="+filepath.Join(testdir, "image.go"), "GOLINE=4", "GOPACKAGE=image", "GOTESTDIR="+testdir, "GOCOVERDIR="+os.Getenv("GOCOVERDIR"))
			if err := cmd.Run(); err != nil {
				t.Error(err)
			}
		...
		t.Run("iota, string", func(t *testing.T) {
			...
			cmd.Env = append(cmd.Environ(), "GOFILE="+filepath.Join(testdir, "image.go"), "GOLINE=18", "GOPACKAGE=image", "GOTESTDIR="+testdir, "GOCOVERDIR="+os.Getenv("GOCOVERDIR"))
			if err := cmd.Run(); err != nil {
				t.Error(err)
			}
	...
	t.Run("when bad go file, then error", func(t *testing.T) {
		...
		cmd := exec.Command(testbin, "--type", "Color")
		cmd.Env = append(cmd.Environ(), "GOFILE=README.md", "GOLINE=5", "GOPACKAGE=image", "GOTESTDIR="+testdir, "GOCOVERDIR="+os.Getenv("GOCOVERDIR"))
		if err := cmd.Run(); err == nil {
			t.Fatal("must be error")
		}
	})
	...
	t.Run("when enum values not immediately after go:generate line, then error", func(t *testing.T) {
		cmd := exec.Command(testbin, "--type", "Color")
		cmd.Env = append(cmd.Environ(), "GOFILE="+filepath.Join(testdir, "image.go"), "GOLINE=1", "GOPACKAGE=image", "GOTESTDIR="+testdir, "GOCOVERDIR="+os.Getenv("GOCOVERDIR"))
		if err := cmd.Run(); err == nil {
			t.Fatal("must be error")
		}
	})
	...
	t.Run("when invalid package name, then error", func(t *testing.T) {
		cmd := exec.Command(testbin, "--type", "Color")
		cmd.Env = append(cmd.Environ(), "GOFILE="+filepath.Join(testdir, "image.go"), "GOLINE=5", "GOPACKAGE=\"", "GOTESTDIR="+testdir, "GOCOVERDIR="+os.Getenv("GOCOVERDIR"))
		if err := cmd.Run(); err == nil {
			t.Fatal("must be error")
		}
	})
	...
	t.Run("when not found file, then error", func(t *testing.T) {
		cmd := exec.Command(testbin, "--type", "Color")
		cmd.Env = append(cmd.Environ(), "GOFILE=asdf.asdf", "GOPACKAGE=image", "GOLINE=5", "GOTESTDIR="+testdir, "GOCOVERDIR="+os.Getenv("GOCOVERDIR"))
		if err := cmd.Run(); err == nil {
			t.Fatal("must be error")
		}
	})
	...
	t.Run("when wrong params, then error", func(t *testing.T) {
		...
		cmd.Env = append(cmd.Environ(), "GOFILE="+filepath.Join(testdir, "image.go"), "GOLINE=5", "GOPACKAGE=color", "GOTESTDIR="+testdir, "GOCOVERDIR="+os.Getenv("GOCOVERDIR"))
		if err := cmd.Run(); err == nil {
			t.Fatal("must be error")
		}
	})
